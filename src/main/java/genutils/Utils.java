...
public class Utils {
     ...
     * @return 
     */
    public static DoubleMatrix createRowMaskMatrix(DoubleMatrix row, int ratingSpread) {//这个是把rating变成二维矩阵
    ...
    public static HashMap<Integer, List<Integer>> summarizeRatings(DoubleMatrix row) {
        ...
        for(int d = 0; d < row.getColumns(); d++) {//猜测是电影的个数  一个用户的评分矩阵   就是一个用户评论movie的个数
            
            double value = row.get(0, d);//这是分数
            ...
            int key = (int)value;
            List<Integer> get = response.get(key);//key是分数啊
            if(get == null) {
     ...
     * @return 
     */
    public static DoubleMatrix softmax(DoubleMatrix x ) {//这个是根据隐藏单元在求可见单元
        
        int rows = x.getRows();//行是分数
        int columns = x.getColumns();//列是movie
        
        DoubleMatrix response = DoubleMatrix.zeros(rows, columns);
        for(int c = 0; c < x.getColumns(); c++ ) {//遍历movie
     ...
     * @return 
     */
    public static DoubleMatrix createRowMatrix(int size, List<Integer> indices) {// List<Integer> indices  评价过电影的id
    ...
    public static org.jblas.DoubleMatrix logistic(org.jblas.DoubleMatrix x) {//根据可见单元  计算隐藏单元的概率
